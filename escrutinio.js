...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btn-recuperar').addEventListener('click', restaurarDesdeBackup);
  document.getElementById('btn-descartar-backup').addEventListener('click', () => {
    limpiarBackupLocal();
...
function descargarPlantilla() {
  ...
  const CABECERA = [
    'Empleado/a', 'DNI/Pasaporte', 'Puesto',
    'Fecha de toma de posesión', 'Fecha de cese',
    'Teléfono', 'Móvil avisos de emergencia',
    'Usuario IdEA', 'Cuenta Google/Microsoft', 'Coordinador',
  ];
  ...
  const wsCand = XLSX.utils.aoa_to_sheet([
    CABECERA,
    // sin cese → incluido/a
    ['García López, Ana', '12345678A', 'Docente', '01/09/2022', '', '600000001', '600000001', 'anagar001', 'anagar001@g.educaand.es', 'No'],
    ['Martínez Ruiz, Carlos', '23456789B', 'Jefatura Estudios', '01/09/2020', '', '600000002', '600000002', 'carmar002', 'carmar002@g.educaand.es', 'Sí'],
    ['Fernández Jiménez, María', '34567890C', 'Orientación', '01/09/2019', '', '600000003', '600000003', 'marfer003', 'marfer003@g.educaand.es', 'No'],
    // cese futuro → incluida
    ['Sánchez Romero, Laura', '45678901D', 'Docente', '01/09/2023', '30/06/2026', '600000004', '600000004', 'lausán004', 'lausán004@g.educaand.es', 'No'],
    // cese pasado → EXCLUIDO (no aparecerá en la aplicación)
    ['Pérez Gómez, Luis', '56789012E', 'Conserje', '01/09/2018', '31/01/2025', '600000005', '600000005', 'luipér005', 'luipér005@g.educaand.es', 'No'],
  ]);
  wsCand['!cols'] = [
  ...
  const wsElec = XLSX.utils.aoa_to_sheet([
    CABECERA,
    // sin cese → incluida
    ['Álvarez Moreno, Carmen', '11111111A', 'Docente', '01/09/2015', '', '611000001', '611000001', 'carálv001', 'carálv001@g.educaand.es', 'No'],
    ['Blanco Ruiz, Javier', '22222222B', 'Docente', '01/09/2017', '', '611000002', '611000002', 'javbla002', 'javbla002@g.educaand.es', 'No'],
    ['Castro López, Dolores', '33333333C', 'Secretaría', '01/09/2010', '', '611000003', '611000003', 'dolcas003', 'dolcas003@g.educaand.es', 'Sí'],
    // cese futuro → incluido
    ['Díaz Fernández, Antonio', '44444444D', 'Orientación', '01/02/2026', '30/06/2026', '611000004', '611000004', 'antdía004', 'antdía004@g.educaand.es', 'No'],
    // cese pasado → EXCLUIDA (no aparecerá en la aplicación)
    ['Escribano García, Lucía', '55555555E', 'Docente', '01/09/2014', '15/01/2025', '611000005', '611000005', 'lucesc005', 'lucesc005@g.educaand.es', 'No'],
  ]);
...
async function iniciarEscrutinio() {
  ...
  if ('showSaveFilePicker' in window) {
    ...
    mostrarToast('Guardado automático en Excel no disponible en este navegador. Se usará copia local.', true);
  }
...
async function exportarExcel() {
  ...
  if (estado.fileHandle) {
    try {
      ...
      return;
    } catch { /* si falla, caer en descarga */ }
  }
...
async function autoGuardar() {
  ...
  try {
    const buf      = XLSX.write(construirLibroResultados(), { bookType: 'xlsx', type: 'array' });
    const writable = await estado.fileHandle.createWritable();
    await writable.write(new Blob([buf]));
    await writable.close();
...
function mostrarIndicadorGuardado(ok) {
  ...
  el.textContent = ok ? `✔ Guardado automáticamente a las ${hora}` : `⚠ Error al guardar (${hora})`;
...
async function restaurarDesdeBackup() {
  try {
    ...
    estado.fechaInicio = data.fechaInicio ? new Date(data.fechaInicio) : new Date();
    ...
    if ('showSaveFilePicker' in window) {
      const slug = data.titulo.replace(/\s+/g, '_').replace(/[^a-zA-Z0-9_\-]/g, '').slice(0, 40);
      try {
      ...
      } catch { /* el usuario canceló — continuamos sin archivo */ }
    }
    ...
    autoGuardar();
    irA('esc');
    mostrarToast(`Votación recuperada: ${estado.historial.length} votos restaurados`);
  } catch {
